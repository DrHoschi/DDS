  ...
  const profiles = {
    FLOOR: {
      ...
      snapPoints: [
        ...
        // WALL placement contract: the WALL base/atlas anchor sits on an actual
        // FLOOR grid corner. Each following corner is one full grid edge away.
        ...
        { id: "corner-east", connectionClass: "FLOOR_CORNER", compatibleClasses: ["MODULE_BASE"], position: { x: 1.3, y: 0, z: 1.3 }, rotationY: 45 },
        { id: "corner-south", connectionClass: "FLOOR_CORNER", compatibleClasses: ["MODULE_BASE"], position: { x: 1.3, y: 0, z: -1.3 }, rotationY: 135 },
        { id: "corner-west", connectionClass: "FLOOR_CORNER", compatibleClasses: ["MODULE_BASE"], position: { x: -1.3, y: 0, z: -1.3 }, rotationY: 225 },
        { id: "corner-north", connectionClass: "FLOOR_CORNER", compatibleClasses: ["MODULE_BASE"], position: { x: -1.3, y: 0, z: 1.3 }, rotationY: 315 },
        { id: "beam-center", connectionClass: "FLOOR_BEAM", compatibleClasses: ["MODULE_BASE"], position: { x: 0.8, y: 0, z: 0 }, rotationY: 0 },
      ],
    },
    WALL: {
      allowedRotations: ROTATIONS,
      snapPoints: [
        { id: "base", connectionClass: "MODULE_BASE", compatibleClasses: ["FLOOR_WALL", "FLOOR_CORNER", "FLOOR_BEAM", "MODULE_SIDE"], position: { x: 0, y: 0, z: 0 }, rotationY: 180 },
    ...
    ROOF: {
      allowedRotations: ROTATIONS,
      snapPoints: [
        { id: "base", connectionClass: "ROOF_BASE", compatibleClasses: ["WALL_TOP", "BEAM_TOP"], position: { x: 0, y: 0, z: 0 }, rotationY: 180 },
      ],
    },
    BEAM: {
      allowedRotations: ROTATIONS,
      snapPoints: [
        { id: "base", connectionClass: "MODULE_BASE", compatibleClasses: ["FLOOR_BEAM", "FLOOR_WALL", "MODULE_SIDE", "WALL_TOP"], position: { x: 0, y: 0, z: 0 }, rotationY: 180 },
...
export class ConstructionPrototypeController {
  ...
  selectPiece(category) {
    if (!MODULE_CATEGORIES.includes(category)) throw new Error(`Unsupported DDS-04F piece category: ${category}`);
  ...
  selectSnapTarget(targetKey) {
    const target = this.#targetCandidates.find((candidate) => candidate.key === targetKey);
    if (!target) throw new Error(`Unknown or unavailable DDS-04F snap target: ${targetKey}`);
    const selectedTarget = this.#selectedPiece === "FLOOR"
      ? this.#targetCandidates.find((candidate) => candidate.targetInstanceId === target.targetInstanceId && candidate.targetSnapId === target.targetSnapId && candidate.transform.rotation.y === this.#rotation) ?? target
      : target;
    this.#selectedTargetKey = selectedTarget.key;
    this.#selectedTargetRequest = { ...selectedTarget.request };
    const selected = this.#placement.previewPlacement({ ...selectedTarget.request, rotation: this.#rotation });
    this.#status = selected.valid ? "Bauplatz gewählt ✓" : messageForInvalid(selected.reason);
    return this.snapshot();
  }

  place() {
    const ghost = this.#placement.snapshot().ghostPreview;
    if (!ghost?.valid) { this.#status = messageForInvalid(ghost?.reason); return this.snapshot(); }
    const committed = this.#placement.placePreview();
    this.#materials.assignMaterial(committed.instanceId, this.#selectedMaterial);
    this.#selectedInstanceId = committed.instanceId;
    this.#nextInstanceNumber += 1;
    this.#clearTargetSelection();
    const placedStatus = `${PIECE_LABELS[this.#selectedPiece]} platziert ✓`;
  ...
  undo() {
    try {
      ...
      if (this.#selectedInstanceId === undone.instanceId) this.#selectedInstanceId = null;
      this.#lastWolfResult = null;
      this.#clearTargetSelection();
      this.#refreshPreview();
  ...
  selectInstance(instanceId) {
    const instance = this.#constructionState.snapshot().instances.find((candidate) => candidate.id === instanceId);
    if (!instance) throw new Error(`Unknown DDS-04F module instance: ${instanceId}`);
  ...
  selectMaterial(materialId) {
    if (!MATERIAL_IDS.includes(materialId)) throw new Error(`Unsupported DDS-04F material: ${materialId}`);
    this.#selectedMaterial = materialId;
    if (this.#selectedInstanceId) {
      this.#materials.assignMaterial(this.#selectedInstanceId, materialId);
  ...
  wolfTest({ source = { x: -4, y: 0, z: 0 }, direction = { x: 1, y: 0, z: 0 }, strength = 8, maxDistance = 12 } = {}) {
    this.#placement.clearPreview();
    this.#lastWolfResult = this.#wolf.apply({ source, direction, strength, maxDistance });
    const wolfStatus = this.#lastWolfResult.failedConnectionIds.length > 0
  ...
  snapshot() {
    ...
    let stability = null;
    if (construction.instances.length > 0 && construction.instances.every((instance) => instance.materialRef)) stability = this.#materials.projectStability();
    return deepFreeze({
      ui: { selectedPiece: this.#selectedPiece, selectedMaterial: this.#selectedMaterial, selectedInstanceId: this.#selectedInstanceId, rotation: this.#rotation, status: this.#status,
        targetSelection: { recommendedTargetKey: this.#recommendedTargetKey, selectedTargetKey: this.#selectedTargetKey, locked: this.#selectedTargetKey !== null, candidates: clone(this.#targetCandidates) } },
      construction: clone(construction), placement: clone(placement), stability: stability ? clone(stability) : null,
      lastWolfResult: this.#lastWolfResult ? clone(this.#lastWolfResult) : null,
    });
  }

  #registerDefinitions() {
    for (const category of MODULE_CATEGORIES) this.#constructionState.registerModuleDefinition({ id: definitionId(category), category });
  }

  #rebuildDerivedAuthorities() {
    this.#placement = new SnapPlacementFoundation({ constructionState: this.#constructionState });
    for (const category of MODULE_CATEGORIES) {
      const profile = snapProfile(category);
      this.#placement.registerSnapProfile({ definitionId: definitionId(category), allowedRotations: profile.allowedRotations, snapPoints: profile.snapPoints });
    }
    this.#materials = new MaterialStabilityFoundation({ constructionState: this.#constructionState });
    this.#wolf = new WolfDynamicResponseFoundation({ constructionState: this.#constructionState });
  }

  #addStarterFloor() {
    const starterId = "floor:starter";
    this.#constructionState.addModuleInstance({ id: starterId, definitionId: definitionId("FLOOR"), placementState: "PLACED",
  ...
  #refreshPreview() {
    ...
    const sourceProfile = placementState.snapProfiles.find((profile) => profile.definitionId === definitionId(this.#selectedPiece));
    this.#targetCandidates = [];
    this.#recommendedTargetKey = null;
    if (!sourceProfile) { this.#placement.clearPreview(); this.#status = "Für dieses Teil fehlt ein Snap-Profil."; return; }
    ...
    for (const targetInstance of construction.instances) {
      if (targetInstance.placementState === "DETACHED") continue;
      const targetProfile = placementState.snapProfiles.find((profile) => profile.definitionId === targetInstance.definitionId);
      if (!targetProfile) continue;
      for (const sourceSnap of sourceProfile.snapPoints) {
        for (const targetSnap of targetProfile.snapPoints) {
          const request = { instanceId: incomingInstanceId, definitionId: definitionId(this.#selectedPiece), sourceSnapId: sourceSnap.id, targetInstanceId: targetInstance.id, targetSnapId: targetSnap.id, rotation: this.#rotation };
          const candidate = this.#placement.previewPlacement(request);
          if (candidate.valid) {
            const key = targetChoiceKey(request);
            this.#targetCandidates.push({ key, request: { ...request }, transform: clone(candidate.transform), targetInstanceId: targetInstance.id, targetSnapId: targetSnap.id, sourceSnapId: sourceSnap.id });
          } else lastInvalid = candidate;
        }
      }
    }
    this.#recommendedTargetKey = this.#targetCandidates[0]?.key ?? null;
    if (this.#selectedTargetRequest) {
      const lockedRequest = { ...this.#selectedTargetRequest, instanceId: incomingInstanceId, definitionId: definitionId(this.#selectedPiece), rotation: this.#rotation };
      const lockedCandidate = this.#placement.previewPlacement(lockedRequest);
      this.#selectedTargetRequest = { ...lockedRequest };
      this.#selectedTargetKey = targetChoiceKey(lockedRequest);
      this.#status = lockedCandidate.valid ? `${PIECE_LABELS[this.#selectedPiece]} bleibt am gewählten Bauplatz ✓` : messageForInvalid(lockedCandidate.reason);
      return;
    }
    ...
    if (!lastInvalid) { this.#placement.clearPreview(); this.#status = "Noch kein passender Einrastpunkt."; return; }

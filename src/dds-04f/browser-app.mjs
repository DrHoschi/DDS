import {
  ...
  MATERIAL_LABELS,
  PIECE_LABELS,
} from "./construction-ui-controller.mjs?build=DDS-FLOOR-CAL-TB1.8";
...
import {
  ...
  snapWorldPosition,
  spriteDescriptor,
} from "../dds-05a/sprite-presentation.mjs?build=DDS-FLOOR-CAL-TB1.8";

const EXPECTED_BUILD_ID = "DDS-FLOOR-CAL-TB1.8";
...
function makeCalibrationCross(position, projection) {
  const element = document.createElement("div");
  const point = scenePoint(position, projection);
  element.dataset.calibration = "grid-center";
  element.setAttribute("aria-hidden", "true");
  element.style.cssText = `position:absolute;left:${point.x}px;top:${point.y}px;width:18px;height:18px;transform:translate(-50%,-50%);z-index:9998;pointer-events:none;background:linear-gradient(transparent 8px,#d62828 8px,#d62828 10px,transparent 10px),linear-gradient(90deg,transparent 8px,#d62828 8px,#d62828 10px,transparent 10px);`;
  return element;
}
